...
const registerHomepageRoutes = (app, pool, authenticateToken) => {
  ...
  app.get('/api/homepage/admin', authenticateToken, requireEditor, async (req, res) => {
    try {
      const row = await readContent();
      res.set('Cache-Control', 'no-store');
  ...
  app.put('/api/homepage/admin', authenticateToken, requireEditor, async (req, res) => {
    let content;
    try {
